<script lang="ts">
	...
	// The emailed link carries the token in the URL fragment (#token=…), which the browser
	// never sends to the server, so it stays out of access logs and Referer headers.
	let token = '';
	...
	async function reset(e: SubmitEvent) {
		...
		try {
			...
			if (res.ok) {
				...
				error = data.error || 'Could not reset your password. Please try again.';
			}
		} catch {
...
<svelte:head><title>Choose a new password — Calnode</title></svelte:head>
...
<div class="flex min-h-screen items-center justify-center bg-muted/30 p-6">
	<div class="w-full max-w-sm">
		...
		{#if phase === 'checking'}
			<p class="text-center text-sm text-muted-foreground">Checking your link…</p>
		{:else if phase === 'invalid'}
			<div class="rounded-md bg-destructive/10 px-4 py-3 text-center text-sm text-destructive">
				This reset link is invalid or has expired. Links work once, for 30 minutes, and only the
				newest one you requested.
			</div>
			...
			<form onsubmit={reset} class="space-y-4">
				{#if accountEmail}
					<div class="space-y-1.5">
						...
						<!-- readonly rather than disabled, so password managers read it as the username -->
						<Input id="email-display" type="email" autocomplete="username" value={accountEmail} readonly />
					</div>
				{/if}
				<div class="space-y-1.5">
					<Label for="password">New password</Label>
					<Input id="password" type="password" autocomplete="new-password" bind:value={password} required minlength={8} />
					<p class="text-xs text-muted-foreground">8 to 72 characters</p>
				</div>
				<div class="space-y-1.5">
				...
				<p class="text-center text-xs text-muted-foreground">
					This signs you out everywhere else and signs you in here.
				</p>
			</form>
		{/if}
	</div>
