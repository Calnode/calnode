<script lang="ts">
	...
	const notesBlockedReason = $derived.by(() => {
		if (notetaker && !notetaker.enabled) {
			return { text: 'The notetaker is turned off.', href: '/admin/settings/video', label: 'Settings → Video' };
		}
		if (notetaker && !notetaker.stt_api_key_set) {
			return { text: 'No Deepgram API key is set.', href: '/admin/settings/video', label: 'Settings → Video' };
		}
		if (llm && !llm.active) {
			return { text: 'No LLM is configured — notes are generated by summarizing the transcript with one.', href: '/admin/settings/ai', label: 'Settings → AI' };
		}
		return null;
	});
			...
			<div class="p-4">
				...
				{#if openNotes === r.id}
					<div class="mt-3 rounded-md border bg-muted/40 p-3">
						...
						{:else}
							<p class="text-xs text-muted-foreground">No notes yet — they appear a few minutes after a recorded meeting. Use Refresh to check again.</p>
						{/if}
					</div>
				{/if}
