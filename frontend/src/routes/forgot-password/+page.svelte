<script lang="ts">
	...
	// null while loading. If the status call itself fails the form is shown anyway: the
	// request endpoint answers the same way whether or not it can send.
	...
	const available = $derived(status === null || (status.smtp_configured && status.email_login));
...
<svelte:head><title>Reset your password — Calnode</title></svelte:head>
	...
	<div class="w-full max-w-sm">
		...
		{:else if !available}
			<div class="rounded-md bg-muted px-4 py-3 text-sm text-muted-foreground">
				Password reset by email isn't available on this server. Ask a workspace admin to set a new
				password for you from the Members page. If you run this server,
				<code class="text-foreground">calnode reset-admin</code> resets any account from the command line.
			</div>
		{:else if sentMessage}
			<div class="space-y-3">
				<div class="rounded-md bg-green-50 px-3 py-2.5 text-sm text-green-700">{sentMessage}</div>
				<p class="text-sm text-muted-foreground">
					The link works once and expires in 30 minutes. If nothing arrives, check your spam folder,
